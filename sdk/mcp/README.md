...
The largest open flight-search MCP server. 53 ready-to-run airline connectors fire in parallel on your machine — Ryanair, EasyJet, Wizz Air, Southwest, AirAsia, and 48 more — plus enterprise GDS/NDC providers (Amadeus, Duffel, Sabre) through the BoostedTravel backend. One tool call scans the entire world for flights, including airlines your agent didn't know existed.
...
- **53 airline connectors** run locally on your machine via Python + Playwright — no API key needed for search
...
- 53 connectors fire in parallel — scans airlines across Europe, Asia, Americas, Middle East, Africa simultaneously
...
1. `search_flights("LON", "BCN", "2026-06-15")` — returns offers with prices from 53 airlines
...
│     │     53 airline connectors via Playwright + httpx        │
...
Just omit `BOOSTEDTRAVEL_API_KEY` from your config. Local search (53 airline connectors) works without any key. You'll only miss the enterprise GDS/NDC sources (Amadeus, Duffel, etc.).
...
## Supported Airlines (53 Connectors)
