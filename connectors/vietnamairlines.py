"""
Vietnam Airlines (VN) — CDP Chrome connector — form fill + API intercept.

Vietnam Airlines's website at www.vietnamairlines.com uses a search widget with autocomplete
...
_USER_DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ".vietnamairlines_chrome_data"
)
...
async def _get_context():
    ...
    async with lock:
        ...
        try:
            ...
            _pw_instance = pw
            logger.info("VietnamAirlines: connected to existing Chrome on port %d", _DEBUG_PORT)
        except Exception:
            if pw:
                try:
                    await pw.stop()
            ...
            logger.info("VietnamAirlines: Chrome launched on CDP port %d", _DEBUG_PORT)
...
class VietnamAirlinesConnectorClient:
    """Vietnam Airlines (VN) CDP Chrome connector."""

    IATA = "VN"
    AIRLINE_NAME = "Vietnam Airlines"
    SOURCE = "vietnamairlines_direct"
    HOMEPAGE = "https://www.vietnamairlines.com/en"
    ...
    async def search_flights(self, req: FlightSearchRequest) -> FlightSearchResponse:
            ...
            try:
                if any(k in url for k in ["/search", "/availability", "/flight",
                    ...
                    if any(k in keys_str for k in ["flight", "itiner", "offer", "fare",
                        ...
                        api_event.set()
                        logger.info("VietnamAirlines: captured API → %s (%d keys)", url[:80], len(data))
            except Exception:
                pass
        ...
        try:
            logger.info("VietnamAirlines: loading homepage for %s→%s", req.origin, req.destination)
            ...
            logger.info("VietnamAirlines: search clicked")
            ...
            logger.info("VietnamAirlines %s→%s: %d offers in %.1fs", req.origin, req.destination, len(offers), elapsed)

            search_hash = hashlib.md5(
                f"vietnamairlines{req.origin}{req.destination}{req.date_from}".encode()
            ).hexdigest()[:12]
    ...
    async def _fill_airport(self, page, direction: str, iata: str) -> bool:
        try:
            ...
            await asyncio.sleep(0.5)
            logger.info("VietnamAirlines: airport %s → %s", direction, iata)
            return True
        except Exception as e:
            logger.warning("VietnamAirlines: airport fill error for %s: %s", iata, e)
            return False
    ...
    async def _fill_date(self, page, req: FlightSearchRequest) -> bool:
        ...
        try:
            ...
            if clicked:
                logger.info("VietnamAirlines: date selected %s", dt.strftime("%Y-%m-%d"))
            await asyncio.sleep(1.0)
            return True
        except Exception as e:
            logger.warning("VietnamAirlines: date error: %s", e)
            return False
    ...
    def _build_offer(self, flight: dict, req: FlightSearchRequest) -> Optional[FlightOffer]:
        try:
            ...
            logger.debug("VietnamAirlines: offer parse error: %s", e)
            return None
    ...
    def _booking_url(self, req: FlightSearchRequest) -> str:
        try:
            ...
            date_str = ""
        return f"https://www.vietnamairlines.com/en?from={req.origin}&to={req.destination}&date={date_str}"

    def _empty(self, req: FlightSearchRequest) -> FlightSearchResponse:
        search_hash = hashlib.md5(f"vietnamairlines{req.origin}{req.destination}{req.date_from}".encode()).hexdigest()[:12]
        return FlightSearchResponse(
            search_id=f"fs_{search_hash}", origin=req.origin, destination=req.destination,
